...
def getCostBasis(queue: PurchasesQueue, txn: CoinbaseTransaction) -> float:
	...
	while True:
		...
		if queue.length == 0 and txn.fees >= (quantityRemaining * txn.spotPriceAtSale):
			# print("This was likely a conversion and missing {} {} was covered by fees.".format(quantityRemaining, txn.assetName))
			...
			# print("Found {}, taking the entire contents and looking for {}.".format(purchase.quantity, quantityRemaining))
			...
			# print("Found {}. modifying the last transaction in place by {} from {}".format(currentQuantity, quantityRemaining, modifiedPurchase.quantity))
			quantityRetrieved += purchase.quantity
